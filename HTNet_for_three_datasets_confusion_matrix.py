...
def computeStrain(u, v):
    ...
    os1 = np.array(np.sqrt(u_x**2 + v_y**2 + 1/2 * (u_y+v_x)**2).ffill(1).ffill(0))
    return os1
...
def get_whole_u_v_os():
    ...
    df = pandas.read_excel('datasets/CASME3/annotation/cas(me)3_part_A_ME_label_JpgIndex_v2_final.xlsx')
    m, n = df.shape
    base_data_src = 'datasets/CASME3/Part_A_ME_clip/frame'
    ori_datasets = 'datasets/casme3_crop_datasets_whole'
    total_emotion=0
    image_size_u_v = 28
    whole_u_v_os_images = []
    ...
    for i in range(0, m):
        ...
        img_path_apex= os.path.join(base_data_src, f"{df['Subject'][i]}_{df['Filename'][i]}_{df['Onset'][i]}", str(df['Apex'][i])+ '.jpg')
        img_path_onset = os.path.join(base_data_src, f"{df['Subject'][i]}_{df['Filename'][i]}_{df['Onset'][i]}", str(df['Onset'][i])+ '.jpg')

        if not os.path.exists(img_path_apex):
            print(f"File {img_path_apex} does not exist")
            whole_u_v_os_images.append(None)
            continue
        if not os.path.exists(img_path_onset):
            print(f"File {img_path_onset} does not exist")
            whole_u_v_os_images.append(None)
            continue
        if img_path_apex == img_path_onset:
            print(f"File {img_path_apex} is the same as {img_path_onset}")
            whole_u_v_os_images.append(None)
            continue
        ...
        mtcnn = MTCNN(margin=0, image_size=side, select_largest=True, post_process=False, device='cuda:0')
        ...
        face_onset = cv2.resize(train_face_image_onset, (224, 224), interpolation=cv2.INTER_LINEAR)
        face_apex = cv2.resize(train_face_image_apex, (224, 224), interpolation=cv2.INTER_LINEAR)
...
def create_norm_u_v_os_train_test():
    ...
    base_destination_folder= 'datasets/casme3_crop_tvl1_norm_u_v_os/'
    base_whole_destination_folder = 'datasets/casme3_crop_tvl1_whole_norm_u_v_os'
    os.makedirs(base_whole_destination_folder, exist_ok=True)
    whole_u_v_os_Arr = get_whole_u_v_os()

    print('finish get')

    emotion_three_class = {'anger': 0, 'disgust': 0, 'fear': 0, 'happy': 1, 'sad': 0, 'surprise': 2}



    df['label_for_3'] = df['emotion'].str.lower().map(emotion_three_class)
    ...
    for i in range(0, m):
        for j in range(0, m):
            if df['Subject'][i] == df['Subject'][j]:
                label_for_three = df['label_for_3'][i]
                if str(label_for_three) == "nan": # emotion is others
                    continue
                saved_filename = str(df['Subject'][i]) + '_' + str(df['Filename'][i]) + '_' + str(
                    df['Apex'][i]) + '.png'
                test_image_path =  base_destination_folder + str(df['Subject'][i]) +'/'+'u_test/'+str(int(label_for_three))+'/'+saved_filename


                u_v_os_image = whole_u_v_os_Arr[i]
                if u_v_os_image is None:
                    continue
                cv2.imwrite(test_image_path, u_v_os_image)
    # #
            else:
                label_for_three = df['label_for_3'][j]
                if str(label_for_three) == "nan": 
                    continue
                saved_filename = str(df['Subject'][j]) + '_'+str(df['Filename'][j])+'_'+str(df['Apex'][j])+'.png'
                test_image_path = base_destination_folder + str(df['Subject'][i]) + '/' + 'u_train/' + str(int(label_for_three))+'/'+saved_filename
                ...
                if u_v_os_image is None:
                    continue
                cv2.imwrite(test_image_path, u_v_os_image)
                cv2.imwrite(os.path.join(base_whole_destination_folder, saved_filename), u_v_os_image)
