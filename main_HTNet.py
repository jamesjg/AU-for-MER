...
def main(config):
    ...
    # subName = [x for x in subName if x.startswith('sub')] # only casme2 test

    all_five_parts_optical_flow = crop_optical_flow_block() # 根据五个部分的坐标，裁剪出四个部分的光流图像
    
    subName = sorted(subName)
    print(subName)
    ...
    for n_subName in subName:
        ...
        for n_expression in expression:
            ...
            # 用three_norm_u_v_os中的图像名，找到上面处理好的STSNet_whole_norm_u_v_os中裁剪后的光流图像
            for n_img in img:
        ...
        for n_expression in expression:
            ...
            for n_img in img:
                # if "sub" not in n_img: #only casme2 train
                #     continue
        ...
        for epoch in range(epochs):
            ...
            #### best result  # 用测试集的准确率来判断最好的模型, 但是也评测了最终模型的准确率
            ...
            print(f"Epoch {epoch + 1}/{epochs} | Train Loss: {train_loss:.4f} | Train Accuracy: {train_acc:.4f} | Test Loss: {val_loss:.4f} | Test Accuracy: {val_acc:.4f}")
